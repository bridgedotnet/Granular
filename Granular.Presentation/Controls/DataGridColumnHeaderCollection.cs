...
namespace System.Windows.Controls
{
    ...
    internal class DataGridColumnHeaderCollection : IEnumerable, INotifyCollectionChanged, IDisposable
    {
        ...
        internal void NotifyHeaderPropertyChanged(DataGridColumn column, DependencyPropertyChangedEventArgs e)
        {
            ...
            NotifyCollectionChangedEventArgs args = NotifyCollectionChangedEventArgs.Replace(e.OldValue, e.NewValue, _columns.IndexOf(column));
        ...
        private void OnColumnsChanged(object sender, NotifyCollectionChangedEventArgs e)
        {
            ...
            switch (e.Action)
            {
                case NotifyCollectionChangedAction.Add:
                    newArgs = NotifyCollectionChangedEventArgs.AddRange(HeadersFromColumns(e.NewItems), e.NewStartingIndex); ;
                    break;

                case NotifyCollectionChangedAction.Remove:                 
                    newArgs = NotifyCollectionChangedEventArgs.RemoveRange(HeadersFromColumns(e.OldItems), e.OldStartingIndex);
                    break;

                case NotifyCollectionChangedAction.Move:
                    newArgs = NotifyCollectionChangedEventArgs.MoveRange(HeadersFromColumns(e.OldItems), e.NewStartingIndex, e.OldStartingIndex);
                    break;

                case NotifyCollectionChangedAction.Replace:
                    newArgs = NotifyCollectionChangedEventArgs.ReplaceRange(HeadersFromColumns(e.OldItems), HeadersFromColumns(e.OldItems), e.OldStartingIndex);
                    break;

                default:
                    newArgs = NotifyCollectionChangedEventArgs.Reset(HeadersFromColumns(e.OldItems), HeadersFromColumns(e.NewItems));
                    break;
            }
        ...
        private static object[] HeadersFromColumns(IEnumerable<object> columns)
        {
            object[] headers = new object[columns.Count()];

            for (int i = 0; i < columns.Count(); i++)
            {
                DataGridColumn column = columns.ElementAt(i) as DataGridColumn;
